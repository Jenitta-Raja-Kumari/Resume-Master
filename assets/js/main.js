...
const setExperience = internship => {
  const expList = document.querySelector('#experienceList');
  internship.forEach(exper => {
...
const setCertification = certifications => {
  ...
  certifications.forEach(evt => {
    ...
    const achItem = document.createElement('div');
    achItem.className = 'ach-item';
    const achTitl = document.createElement('div');
    achTitl.className = 'ach-title';
    achTitl.innerHTML = evt.title;
    achItem.appendChild(achTitl);
    const achTitle = document.createElement('div');
    achTitle.className = 'ach-title';
    achTitle.innerHTML = evt.desc;
    achTitl.appendChild(achTitle);
...
const setDeclaration =declaration =>{
  document.querySelector('#aboutdec').innerHTML =declaration.desc ;
  document.querySelector('#aboutplace').innerHTML =declaration.place ;
  var today=new Date();
  document.querySelector('#aboutdate').innerHTML ='Date: '+today.getDate()+'-'+(today.getMonth()+1)+'-'+today.getFullYear();
...
(() => {
  ...
  setExperience(profileData.internship);
  setProjects(profileData.projects);
  setSkills(profileData.skills);
  setEducation(profileData.education);
  setCertification(profileData.certifications);
  setDeclaration(profileData.declaration);
